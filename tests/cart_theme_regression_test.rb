...
assert(card_js.include?("formData.set('sections', 'ona_header')"), 'shared card add does not request the drawer section')
assert(card_js.include?("new CustomEvent('cart:add'"), 'shared card controller does not emit native cart add event')
assert(card_js.include?("new CustomEvent('cart:update'"), 'shared card controller does not emit compatibility cart update event')
assert(card_js.include?('sections: response.sections || {}'), 'shared card update event lacks section HTML')
assert(card_js.include?('submittedVariantId = variantInput.value'), 'shared card events can report a variant changed in flight')
assert(card_js.scan(/sourceId: submittedVariantId/).length == 2, 'shared card events do not preserve the submitted variant ID')

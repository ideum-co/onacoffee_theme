...
assert(card.match?(/<article\s+class="ona-card"/), 'shared ONA card root is not a semantic article')
...
assert(!card.include?("'ona-product-card.css' | asset_url | stylesheet_tag"), 'shared card repeats its stylesheet per product')
assert(!card.include?("'ona-product-card.js' | asset_url"), 'shared card repeats its controller per product')
...
assert(card.include?('type="radio"') && card.include?('data-card-radio'), 'card variants lack native radio behavior')
assert(card.match?(/data-card-radio[\s\S]{0,700}\{% unless variant\.available %\}disabled\{% endunless %\}/), 'sold-out variant radios remain operable')
...
assert(card_js.include?("document.addEventListener('change'"), 'shared card radio controller is not delegated')
...
assert(card_js.include?("document.querySelector('cart-items-component[data-drawer][data-section-id]')"), 'shared card does not resolve a usable drawer section')
assert(card_js.include?("typeof drawer.open !== 'function'"), 'shared card does not verify drawer operability')
assert(!card_js.include?("|| 'ona_header'"), 'shared card cancels native fallback for a missing drawer')
assert(card_js.include?("formData.set('sections', sectionId)"), 'shared card add does not request the resolved drawer section')
assert(card_js.include?('mutationQueue = mutationQueue.then'), 'shared card mutations are not globally serialized')
assert(card_js.include?("new CustomEvent('cart:error'"), 'shared card lacks the native accessible error event')
assert(card_js.include?('window.location.assign(cartUrl)'), 'ambiguous card adds do not reconcile through the cart')
...
assert(card_js.include?("sections[sectionId]"), 'shared card does not validate returned drawer section HTML')

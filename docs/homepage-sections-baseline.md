...
Both sections are ported and now committed here (`sections/club-invite.liquid`,
`sections/club-benefits.liquid`, `snippets/club-invite-typography-style.liquid`), but they
are not on the home page. Three reasons:
...
Until 2026-09-23 those three files existed **only in the live theme**, never in this repo,
so a theme built from a clean checkout would not have contained them and the sentence above
would have dead-ended. They are committed now. The 19 editor labels are a separate matter:
like the Press quotes strings, they live in `locales/*.schema.json`, which is generated by
Shopify's admin language editor and still differs from live — see the locales issue.
